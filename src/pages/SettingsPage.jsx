...
import { Settings as SettingsIcon, User, Bell, Shield, Globe, ChevronRight, Check } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const settingsGroups = [
    {
      ...
      items: [
        ...
        { label: 'Change Password', sub: 'Update your admin credentials', type: 'link' },
        { label: 'Two-Factor Authentication', sub: 'Enhance account security', type: 'toggle', default: true },
      ],
    },

    {
      ...
      icon: Shield,
      items: [
        { label: 'Session Management', sub: 'View and revoke active sessions', type: 'link' },
        { label: 'Audit Log', sub: 'Track all admin actions', type: 'link' },
        { label: 'IP Whitelist', sub: 'Restrict admin access by IP', type: 'toggle', default: false },
      ],
    },
    {
      title: 'Regional',
  ...
  return (
    <PageTransition>
      <TopBar title={t('settings')} subtitle="Manage your account, notifications, and system preferences." />
      ...
      <div className="p-6 max-w-4xl mx-auto flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 space-y-6">
          {settingsGroups.filter(g => g.title === activeTab).map((group, gi) => (
            <motion.div
              key={group.title}
              ...
              <div className="divide-y divide-primary-50">
                {group.items.map((item, ii) => (
                  <div key={item.label} className="p-6 flex items-center justify-between group">
                    ...
                    {item.type === 'toggle' && (
                      <button className={`w-11 h-6 rounded-full p-1 transition-all ${item.default ? 'bg-teal' : 'bg-primary-200'}`}>
                        <div className={`w-4 h-4 bg-white rounded-full transition-all ${item.default ? 'translate-x-5' : 'translate-x-0'}`} />
                      </button>
                    )}
